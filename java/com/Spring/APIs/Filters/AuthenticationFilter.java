package com.Spring.APIs.Filters;
...
public class AuthenticationFilter implements Filter {
	...
	private void executeFilterLogic(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if(token == null || ! authService.validateToken(token)) {
			sendErrorResponse(httpResponse, HttpServletResponse.SC_UNAUTHORIZED,"Unauthorized : Invalid user or missing token");
			return;
		}
	...
	private String getAuthTokenFromCookies(HttpServletRequest request) {
		...
		System.err.println("No cookies recieved in the request");
		return null;
	}
